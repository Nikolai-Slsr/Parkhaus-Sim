...
{
    vehicle **p_parkhaus = calloc(number_of_parking_spaces + 1, sizeof(struct vehicle*)); //creates the main Array which contains pointer to vehicle structs
    if(p_parkhaus == NULL)
    {
        return NULL;
    }
    vehicle *p_end_point = malloc(sizeof(vehicle)); //The 'end_point' has a unique ID(-1) and is always saved at the Last Spot of the Array. It is used to loop through the Array, without needing information about its length beforehand. 
    if(p_end_point == NULL)
    {
        free(p_parkhaus);
        return NULL;
    }
    p_end_point->vehicle_id = -1;
    p_parkhaus[number_of_parking_spaces] = p_end_point; //save "end_point" at the last Index of p_parkhaus 
    return p_parkhaus;
...
int parkhaus_is_Full(vehicle *p_parkhaus[])
{
    ...
    {
        if(p_parkhaus[i] == NULL)   //check if Spot is empty
        { 
            return -1;              //if one Spot is empty then it is not full
        }   
        i++;
    }
...
int park_Car(vehicle *p_parkhaus[], vehicle *p_car, int current_time)
{
    ...
    return -1; //if no free Spot was found then return -1 to indicate failure
}
...
int parkhaus_get_used_Spots(vehicle *p_parkhaus[])
{
    ...
    { 
        if(p_parkhaus[i] != NULL)
        {
            used_spaces++; //for every Spot which isnt empty the counter increases
        }
        i++;
    }
    return used_spaces;
...
{ 
    ...
    free(p_parkhaus[i]);   //frees the memory of the end_pointer
    p_parkhaus[i] = NULL;
